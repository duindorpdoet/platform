...
import { AlertTriangle, CheckCircle2, Database, LifeBuoy, MapPinned, RefreshCw, ShieldCheck, Upload, WalletCards } from "lucide-react";
...
type LiveRun = { groupId: string; groupCode: string; groupVersion: number; leaderEmail: string; runId: string; runStatus: "live" | "paused"; runVersion: number; stopId: string; portalName: string };
type PaymentRow = { id: string; reference: string; registrationReference: string; amountCents: number; netCollectedCents: number; status: string; version: number; reportedAt?: string | null; updatedAt: string };
...
export function AdminConsole({ eventSlug }: { eventSlug: string }) {
  ...
  const [section, setSection] = useState<"overview" | "imports" | "payments" | "planner" | "live">("overview");
  ...
  async function loadPayments() {
    const client = createClient(); if (!client) return;
    const { data, error } = await client.schema("api").rpc("admin_payments_snapshot", { _event_slug: eventSlug });
    if (error) return setNotice("Het betaaloverzicht is alleen beschikbaar voor betaalbeheer.");
    setPayments(data as PaymentRow[]);
  }

  async function paymentCommand(payment: PaymentRow, command: "confirm" | "refund") {
    const defaultCents = command === "refund" ? payment.netCollectedCents : Math.max(0, payment.amountCents - payment.netCollectedCents);
    const amountInput = window.prompt(`${command === "refund" ? "Werkelijk terugbetaald" : "Werkelijk ontvangen"} bedrag in euro`, (defaultCents / 100).toFixed(2).replace(".", ","))?.trim();
    if (!amountInput || !/^\d+(?:[,.]\d{1,2})?$/.test(amountInput)) return setNotice("Vul een geldig positief bedrag met maximaal twee decimalen in.");
    const amountCents = Math.round(Number(amountInput.replace(",", ".")) * 100);
    const externalReference = window.prompt("Externe Tikkie-/bankreferentie:")?.trim();
    const reason = window.prompt("Verplichte auditreden:")?.trim();
    if (!externalReference || !reason || reason.length < 5 || amountCents <= 0) return setNotice("Bedrag, externe referentie en minimaal vijf tekens reden zijn verplicht.");
    if (!window.confirm(`${command === "refund" ? "Terugbetaling" : "Betaling"} van € ${(amountCents / 100).toFixed(2).replace(".", ",")} append-only vastleggen?`)) return;
    const client = createClient(); if (!client) return;
    const key = crypto.randomUUID();
    const requestHash = await digest({ paymentId: payment.id, version: payment.version, amountCents, externalReference, reason, command });
    const result = command === "confirm"
      ? await client.schema("api").rpc("payment_confirm_versioned", { _payment_request_id: payment.id, _expected_version: payment.version, _amount_cents: amountCents, _external_reference: externalReference, _reason: reason, _idempotency_key: key, _request_hash: requestHash })
      : await client.schema("api").rpc("payment_refund", { _payment_request_id: payment.id, _amount_cents: amountCents, _external_reference: externalReference, _reason: reason, _idempotency_key: key, _request_hash: requestHash });
    setNotice(result.error ? `Betaalmutatie geweigerd: ${result.error.message}` : "Betaalmutatie append-only opgeslagen en geaudit.");
  ...
  async function reassignLeader(run: LiveRun) {
    if (supportReason.trim().length < 10) return setNotice("Leg voor een leiderwissel minimaal tien tekens reden vast.");
    const email = window.prompt("Geverifieerd accountadres van de nieuwe groepsleider:", run.leaderEmail)?.trim().toLowerCase();
    if (!email || !/^\S+@\S+\.\S+$/.test(email) || email === run.leaderEmail) return setNotice("Kies een ander geldig en geverifieerd accountadres.");
    if (!window.confirm(`Leider van ${run.groupCode} direct vervangen door ${email}? De oude sessie verliest meteen mutatierecht.`)) return;
    const client = createClient(); if (!client) return;
    const { error } = await client.schema("api").rpc("admin_assign_group_leader_by_email", {
      _group_id: run.groupId,
      _new_leader_email: email,
      _expected_group_version: run.groupVersion,
      _reason: supportReason,
    });
    setNotice(error ? `Leiderwissel geweigerd: ${error.message}` : "Leider vervangen; de vorige leider heeft direct geen mutatierecht meer.");
