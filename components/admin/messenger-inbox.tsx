    ...
    <div className="ticket-layout">
      <div className="ticket-list" aria-label="Gesprekken">
        ...
        {conversations.map((conversation) => <button key={conversation.id} className={selectedId === conversation.id ? "active" : ""} onClick={() => setSelectedId(conversation.id)}>
          <span><strong>{[conversation.systemCode, conversation.displayName || conversation.subjectLabel].filter(Boolean).join(" · ") || "Privégesprek"}</strong><small>{statusLabel[conversation.status]} · {new Date(conversation.updatedAt).toLocaleString("nl-NL")}</small></span>
