  ...
  return <div className="dashboard-stack start-schedule-board">
    ...
    <section className="panel"><p className="kicker">Geen vast aantal huizen</p><h2>Groepen en startafspraken</h2><p>Het voorstel maakt alleen groepen en reserveert finale-instroom. Gewone poorten worden pas live, één voor één, door de server gekozen.</p>
      ...
      {proposal?.groups.map((group: PlannedGroup) => {
        ...
        return <article className={`schedule-card match-${group.preferenceMatch}`} key={group.key}><div><strong>{group.key} · {group.childCount} kinderen</strong><p>{matchLabels[group.preferenceMatch]} · stop gewone poorten {new Date(group.effectiveStopAt).toLocaleTimeString("nl-NL", { hour: "2-digit", minute: "2-digit", timeZone: "Europe/Amsterdam" })} · finale circa {new Date(group.expectedFinaleArrivalAt).toLocaleTimeString("nl-NL", { hour: "2-digit", minute: "2-digit", timeZone: "Europe/Amsterdam" })}</p><small>{registrations.map((registration) => `${registration.reference}: start ${registration.preferredStartAt ? new Date(registration.preferredStartAt).toLocaleTimeString("nl-NL", { hour: "2-digit", minute: "2-digit", timeZone: "Europe/Amsterdam" }) : "maakt niet uit"}, einde ${registration.desiredEndAt ? new Date(registration.desiredEndAt).toLocaleTimeString("nl-NL", { hour: "2-digit", minute: "2-digit", timeZone: "Europe/Amsterdam" }) : "niet gekozen"}, ${registration.paymentEligible ? "betaald" : "betaling open"}`).join(" · ")}</small>{group.warnings.includes("PAYMENT_NOT_CONFIRMED") && <small>Conceptraming: betaling nog niet bevestigd; publicatie blokkeert.</small>}</div><UsersRound /></article>;
      })}
      {snapshot.groups.filter((group) => group.routeMode === "dynamic" && group.schedule).map((group) => <article className={`schedule-card match-${group.schedule!.preferenceMatch}`} key={group.id} draggable onDragStart={(event) => event.dataTransfer.setData("text/group-id", group.id)}><div><strong>{group.code} · {group.childCount} kinderen · revisie {group.schedule!.revision}</strong><p>{matchLabels[group.schedule!.preferenceMatch]} · {group.schedule!.state} · finale {new Date(group.schedule!.expectedFinaleArrivalAt).toLocaleTimeString("nl-NL", { hour: "2-digit", minute: "2-digit", timeZone: "Europe/Amsterdam" })}</p><small>{group.registrations.map((registration) => `${registration.reference}: start ${registration.preferredStartAt ? new Date(registration.preferredStartAt).toLocaleTimeString("nl-NL", { hour: "2-digit", minute: "2-digit", timeZone: "Europe/Amsterdam" }) : "maakt niet uit"}, einde ${registration.desiredEndAt ? new Date(registration.desiredEndAt).toLocaleTimeString("nl-NL", { hour: "2-digit", minute: "2-digit", timeZone: "Europe/Amsterdam" }) : "niet gekozen"}, ${registration.paymentEligible ? "betaald" : "betaling open"}`).join(" · ")}</small><label className="field"><span>Startpunt en tijd (toetsenbord/formulier)</span><select value={group.schedule!.startSlotId} onChange={(event) => void moveGroup(group, event.target.value)}>{snapshot.startPoints.flatMap((point) => point.slots.map((slot) => <option key={slot.id} value={slot.id}>{point.name} · {new Date(slot.startsAt).toLocaleTimeString("nl-NL", { hour: "2-digit", minute: "2-digit", timeZone: "Europe/Amsterdam" })}</option>))}</select></label></div>{group.schedule!.state === "published" ? <CheckCircle2 /> : <Clock3 />}</article>)}
      <div className="separator" /><h3>Voorspelde druk op de laatste poort</h3>{snapshot.finaleFlow.length ? snapshot.finaleFlow.map((row) => <div className="summary-row" key={row.window}><span>{new Date(row.window).toLocaleTimeString("nl-NL", { hour: "2-digit", minute: "2-digit", timeZone: "Europe/Amsterdam" })}</span><strong>{row.groups} groepen · {row.children} kinderen</strong></div>) : <p>Nog geen concept- of gepubliceerde finalevensters.</p>}
    </section>
  </div>;
}
