...
export function GroupExperience({ groupId, userId }: { groupId: string; userId: string }) {
  ...
  return <div className="group-app">
    ...
    {run && !["completed", "stopped"].includes(run.status) && stop && <>
      ...
      <div className="live-grid"><section className="active-portal" aria-live="polite"><img src="/images/pluvierstraat.webp" alt="Sfeervol verlichte poort" /><div className="active-portal-copy"><p className="kicker">Huidige poort · {stop.portal.world}</p><h2 ref={portalHeading} tabIndex={-1}>{stop.portal.name}</h2><p>{stop.portal.description}</p><div className="portal-meta"><span><MapPin size={16} />{stop.portal.address}, {stop.portal.postalCode}</span><span>Spanning {stop.portal.intensity}/4</span></div>{stop.portal.operationStatus !== "open" && <div className="form-warning">Deze poort is tijdelijk {stop.portal.operationStatus}. Volg de instructie van de groepsleiding.</div>}</div></section><aside className="panel live-map-column participant-route-map"><div><p className="kicker">Duindorp nachtkaart · schematisch</p><h3>Jullie spoor door de wijk</h3></div><div className="route-constellation" aria-label={`${run.history.length} afgeronde poorten en de huidige vrijgegeven poort`}><span className="route-street street-a" /><span className="route-street street-b" />{run.history.map((item, index) => <div className={`route-map-stop visited stop-${(index % 6) + 1}`} key={item.sequence}><Check size={13} /><span>{item.world}<small>{item.portalName}</small></span></div>)}<div className={`route-map-stop current stop-${(run.history.length % 6) + 1}`}><MapPin size={15} /><span>Nu · {stop.portal.world}<small>{stop.portal.name}</small></span></div><div className={`route-map-stop locked stop-${((run.history.length + 1) % 6) + 1}`}><LockKeyhole size={13} /><span>Volgende poort</span></div></div><div className="route-instruction"><MapPin /><span><strong>Loop samen naar de vrijgegeven poort.</strong>Bevestig het bezoek daar met de bestaande QR- of korte poortcode; nooit met GPS.</span></div><div className="next-hidden"><LockKeyhole /><span><strong>De volgende stop blijft nog een verrassing.</strong>Rond deze poort samen af. Daarna verschijnt precies één nieuwe bestemming.</span></div></aside></div>
