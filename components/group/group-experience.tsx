...
export function GroupExperience({ groupId, userId }: { groupId: string; userId: string }) {
  ...
  return <div className="group-app">
    ...
    {!run && <div className="pre-event"><div className="pre-art"><img src="/images/avondloop-hero.webp" alt="Verlichte Duindorpse straat in de avond" /><div><p className="kicker">Route nog vergrendeld</p><h2>{snapshot.group.start ? new Date(snapshot.group.start.startsAt).toLocaleString("nl-NL", { dateStyle: "long", timeStyle: "short", timeZone: "Europe/Amsterdam" }) : "Starttijd volgt"}</h2><p>{snapshot.group.start ? `${snapshot.group.start.locationName} · ${snapshot.group.start.address}` : "De organisatie publiceert de startlocatie later."}</p></div></div><div className="panel"><h2>Controleer aanwezigheid</h2>{snapshot.access.leader ? <>{roster.map((item) => <label className="checkfield" key={item.registrationChildId}><input type="checkbox" checked={present.has(item.registrationChildId)} onChange={(event) => setPresent((current) => { const next = new Set(current); if (event.target.checked) next.add(item.registrationChildId); else next.delete(item.registrationChildId); return next; })} />{item.firstName} <small>· {item.householdLabel}</small></label>)}<button className="btn full" disabled={busy || offline || present.size === 0} onClick={() => void command("run_start", { _group_id: groupId, _present_registration_child_ids: [...present], _expected_group_version: snapshot.group.version, _idempotency_key: crypto.randomUUID(), _request_hash: [...present].sort().join(":") })}>Start groep met {present.size} aanwezig</button></> : <p>Alleen de aangewezen groepsleider kan de route starten. Tot die tijd blijven adressen verborgen.</p>}</div></div>}
