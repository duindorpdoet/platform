...
import { Camera, Check, CloudOff, LockKeyhole, MapPin, QrCode, RefreshCw, SkipForward } from "lucide-react";
...
export function GroupExperience({ groupId, userId }: { groupId: string; userId: string }) {
  ...
  if (!snapshot) return <div className="panel loading-state"><RefreshCw className="spin" /><p>Bevoegde groepsinformatie ophalen…</p></div>;
  ...
  return <div className="group-app">
    ...
    {run && !["completed", "stopped"].includes(run.status) && stop && <>
      <div className="live-topbar"><span>Stop {stop.sequence}</span><span>{run.remainingStopCount} vergrendeld</span><span>Serverversie {run.version}</span><span>Bevestigd {new Date(run.lastServerConfirmation).toLocaleTimeString("nl-NL")}</span>{snapshot.access.leader && <button className="text-link" disabled={offline || busy} onClick={() => { const reason = run.status === "paused" ? "Groepsleider hervat de route" : window.prompt("Waarom pauzeer je de groep?")?.trim(); if (reason) void command("run_set_state", { _run_id: run.id, _state: run.status === "paused" ? "live" : "paused", _expected_version: run.version, _reason: reason }); }}>{run.status === "paused" ? "Route hervatten" : "Route pauzeren"}</button>}</div>
    ...
    {run && run.history.length > 0 && <div className="panel timeline"><p className="kicker">Logboek</p><h2>Afgeronde poorten</h2>{run.history.map((item) => <div className="summary-row" key={item.sequence}><span>{item.sequence}. {item.portalName} · {item.world}</span><strong>{item.outcome === "all_skipped" ? "Overgeslagen" : item.outcome === "mixed" ? "Deels bezocht" : "Bezocht"}</strong></div>)}</div>}
  </div>;
}
  ...
  useEffect(() => {
    ...
    void import("@zxing/browser").then(async ({ BrowserQRCodeReader }) => {
      try {
        ...
        controls = await reader.decodeFromVideoDevice(undefined, video.current!, (result) => { if (result && !stopped) onResult(result.getText()); });
      } catch { setError("Camera niet beschikbaar. Gebruik de korte code."); }
    });
