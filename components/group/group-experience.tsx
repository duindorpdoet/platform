...
import "./group-experience.css";
import { AlertTriangle, Camera, Check, CloudOff, Footprints, LockKeyhole, MapPin, QrCode, RefreshCw, SkipForward } from "lucide-react";
...
export function GroupExperience({ groupId, userId }: { groupId: string; userId: string }) {
  ...
  return <div className="group-app group-premium">
    ...
    <div className="group-route-context"><span className="group-route-code"><Footprints aria-hidden="true" />{snapshot.group.code}</span><span className={`group-route-phase ${run?.status === "live" && !offline ? "is-active" : ""}`}>{offline ? "Laatste opgeslagen opdracht" : !run ? "Voor de tocht" : run.status === "completed" ? "Tocht afgerond" : run.status === "stopped" ? "Tocht beëindigd" : run.status === "paused" ? "Route gepauzeerd" : "De tocht is begonnen"}</span><button type="button" className="group-route-refresh" onClick={() => void load()} aria-label="Groepsinformatie vernieuwen"><RefreshCw aria-hidden="true" /><span>Vernieuwen</span></button></div>
    ...
    {run && !["completed", "stopped"].includes(run.status) && stop && <>
      <div className="live-topbar group-route-metrics"><span>{isFinale ? "Laatste poort" : `Poort ${stop.sequence}`}</span><span>{elapsedMinutes} min onderweg</span><span>Geen nieuwe gewone poort vanaf {snapshot.group.effectiveOrdinaryStopAt ? new Date(snapshot.group.effectiveOrdinaryStopAt).toLocaleTimeString("nl-NL", { hour: "2-digit", minute: "2-digit", timeZone: "Europe/Amsterdam" }) : "de afgesproken grens"}</span><span>Verwachte laatste poort {snapshot.group.expectedFinaleArrivalAt ? new Date(snapshot.group.expectedFinaleArrivalAt).toLocaleTimeString("nl-NL", { hour: "2-digit", minute: "2-digit", timeZone: "Europe/Amsterdam" }) : "wordt berekend"}</span><span>Bijgewerkt om {new Date(run.lastServerConfirmation).toLocaleTimeString("nl-NL", { hour: "2-digit", minute: "2-digit", timeZone: "Europe/Amsterdam" })}</span>{snapshot.access.leader && <button className="text-link" disabled={offline || busy} onClick={() => void command("run_set_state", { _run_id: run.id, _state: run.status === "paused" ? "live" : "paused", _expected_version: run.version, _reason: run.status === "paused" ? "Groepsleider hervat de route" : "Groepsleider pauzeert de route" })}>{run.status === "paused" ? "Route hervatten" : "Route pauzeren"}</button>}</div>
