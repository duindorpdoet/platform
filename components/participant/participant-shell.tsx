...
import { ChildPaymentRows, childPaymentFor, childPaymentState, type PaymentChild } from "@/components/payments/child-payment-rows";
...
type RegistrationSnapshot = {
  ...
  registration?: {
    ...
    togetherCode: string;
    version: number;
    children: Array<PaymentChild & { ageAtEvent?: number | null }>;
...
function WalkerNow({ context, registration, group, reload }: { context: ParticipantContext; registration: RegistrationSnapshot | null; group: GroupSnapshot | null; reload: () => Promise<void> }) {
  ...
  if (run && ["live", "paused"].includes(run.status)) {
    ...
    return <ParticipantPageFrame eyebrow="De tocht is begonnen" title={run.status === "paused" ? "Even stilstaan." : "Dit is jullie volgende poort."}>
      ...
      {stop && <section className="participant-hero-card live"><img src="/images/pluvierstraat.webp" alt="Nachtelijk verlichte Duindorpse poort" /><div className="participant-hero-overlay"><p className="participant-eyebrow">{isFinale ? "Laatste poort" : `Vrijgegeven · ${stop.portal.world}`}</p><h2>{stop.portal.name}</h2><p>{stop.portal.address}, {stop.portal.postalCode}</p><small>{Math.floor((run.elapsedSeconds ?? 0) / 60)} minuten onderweg · {isFinale ? "hierna is de tocht afgerond" : "de volgende poort volgt na bevestiging"}</small><Link className="btn participant-primary-action" href="/omgeving/meeloper/route">{group?.access.leader ? stop.scanAccepted ? isFinale ? "Laatste poort afronden" : "Bezoek afronden" : "Poort bevestigen" : "Bekijk route-instructie"}<ChevronRight /></Link></div></section>}
