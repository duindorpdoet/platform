...
export function PaymentDetails({ payment }: { payment: ParticipantPayment }) {
  ...
  return <section className="payment-details" aria-label={shared ? "Gezamenlijke betaling" : "Tikkie betalen"}>
    ...
    {batch && <p className="payment-payer"><strong>Betaling loopt via {batch.payerName}.</strong>{!batch.canPay && " Je hoeft zelf geen Tikkie te betalen. Jullie inschrijving is in dit gezamenlijke bedrag inbegrepen."}</p>}
    {needsReview ? <p role="status">De inschrijving of betaling is gewijzigd. De organisatie controleert het gezamenlijke verzoek. Gebruik de eerdere Tikkie-link niet.</p> : reported ? <p role="status">De betaling is gemeld en wordt gecontroleerd. Betaal niet nogmaals.</p> : settled ? <p>De betaling is afgehandeld.</p> : <p>De organisatie bevestigt de ontvangst na controle.</p>}
    {canPay && <a className="btn participant-primary-action" href={url!} target="_blank" rel="noreferrer noopener">{shared ? "Betaal het gezamenlijke Tikkie" : "Betaal via Tikkie"}<ExternalLink aria-hidden="true" /></a>}
  </section>;
}
