...
export function PortalDashboard({ eventSlug }: { eventSlug: string }) {
  ...
  if (["draft", "changes_requested"].includes(snapshot.application.status)) return <div className="panel"><h2>Werk jullie poort uit</h2><p>De basisgegevens zijn opgeslagen. Voeg de beleving, beschikbaarheid en andere details toe en dien de plek in voor beoordeling.</p>{snapshot.application.reviewFeedback && <p>{snapshot.application.reviewFeedback}</p>}<Link className="btn" href="/huis-aanmelden">Aanmelding aanvullen</Link></div>;
  if (!snapshot.portal) return <div className="panel empty-state"><Clock /><h2>Aanmelding in beoordeling</h2><p>De organisatie bekijkt jullie idee. Zodra de plek is goedgekeurd, zie je hier alles wat je voor de avond nodig hebt.</p>{snapshot.application.reviewFeedback && <div className="form-warning">Terugkoppeling: {snapshot.application.reviewFeedback}</div>}</div>;
