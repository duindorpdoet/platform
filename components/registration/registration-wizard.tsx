...
type Draft = { adult: { name: string; phone: string }; children: ChildDraft[]; togetherCode: string; startPreference: "early" | "indifferent" | "later"; ordinaryStopAt: string; marketingConsent: boolean };
...
const emptyDraft: Draft = { adult: { name: "", phone: "" }, children: [{ name: "", age: "", accessibilityNote: "" }], togetherCode: "", startPreference: "indifferent", ordinaryStopAt: "", marketingConsent: false };
...
export function RegistrationWizard({ eventSlug, canSubmit }: { eventSlug: string; canSubmit: boolean }) {
  ...
  const [globalStopAt, setGlobalStopAt] = useState<string | null>(null);
  ...
  useEffect(() => {
    ...
    void Promise.all([
      ...
      const preference = preferenceResult.data as { event?: { allowedStopTimes?: string[]; globalOrdinaryStopAt?: string | null } } | null;
      setStopOptions(preference?.event?.allowedStopTimes ?? []);
      setGlobalStopAt(preference?.event?.globalOrdinaryStopAt ?? null);
    });
  }, [eventSlug]);
  ...
  return <div className="wizard panel">
    ...
    {step === 0 && <section><h2 ref={heading} tabIndex={-1}>Wie loopt er mee?</h2><label className="field"><span>Naam verantwoordelijke volwassene</span><input value={draft.adult.name} onChange={(event) => setDraft({ ...draft, adult: { ...draft.adult, name: event.target.value } })} autoComplete="name" /></label><label className="field"><span>Telefoonnummer voor de avond</span><input value={draft.adult.phone} onChange={(event) => setDraft({ ...draft, adult: { ...draft.adult, phone: event.target.value } })} inputMode="tel" autoComplete="tel" /></label><label className="field"><span>Voorkeur voor starten</span><select value={draft.startPreference} onChange={(event) => setDraft({ ...draft, startPreference: event.target.value as Draft["startPreference"] })}><option value="early">Vroeg</option><option value="indifferent">Maakt niet uit</option><option value="later">Later</option></select><small>Dit is een voorkeur. De organisatie bevestigt later één exact startpunt en tijdstip.</small></label><label className="field"><span>Wanneer stoppen jullie met gewone poorten?</span><select value={draft.ordinaryStopAt} onChange={(event) => setDraft({ ...draft, ordinaryStopAt: event.target.value })}><option value="">Tot de algemene grens{globalStopAt ? ` (${new Date(globalStopAt).toLocaleTimeString("nl-NL", { hour: "2-digit", minute: "2-digit" })})` : ""}</option>{stopOptions.map((value) => <option key={value} value={value}>{new Date(value).toLocaleTimeString("nl-NL", { hour: "2-digit", minute: "2-digit" })}</option>)}</select><small>Na dit moment krijgen jullie geen nieuwe gewone poort. De wandeling naar de laatste poort en de eindshow volgen daarna nog.</small></label></section>}
