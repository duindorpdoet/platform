...
type Draft = { adult: { name: string; householdLabel: string; phone: string }; children: ChildDraft[]; togetherCode: string; marketingConsent: boolean };
...
const emptyDraft: Draft = { adult: { name: "", householdLabel: "", phone: "" }, children: [{ name: "", age: "", accessibilityNote: "" }], togetherCode: "", marketingConsent: false };
...
export function RegistrationWizard({ eventSlug, canSubmit }: { eventSlug: string; canSubmit: boolean }) {
  ...
  useEffect(() => {
    ...
    void client.schema("api").rpc("registration_snapshot", { _event_slug: eventSlug }).then(({ data }: { data: unknown }) => {
      ...
      if (snapshot?.draft?.payload) { setDraft(snapshot.draft.payload); setVersion(snapshot.draft.version); }
  ...
  async function save(nextStep?: number) {
    const client = createClient();
    if (!client) return setNotice("De veilige gegevensverbinding is niet geconfigureerd.");
    if (!draft.adult.name.trim() || !draft.adult.householdLabel.trim()) return setNotice("Vul je naam en huishoudnaam in.");
    ...
    setBusy(false);
    if (error) return setNotice(error.message.includes("STALE_VERSION") ? "Deze inschrijving is elders gewijzigd. Vernieuw de pagina." : "Opslaan is niet gelukt. Controleer de velden.");
  ...
  async function submit() {
    if (!canSubmit) return setNotice("De inschrijving is nog niet geopend.");
    if (draft.children.some((child) => !child.name.trim() || !/^\d{1,2}$/.test(child.age))) return setNotice("Vul voor ieder kind een naam en geldige leeftijd in.");
    await save();
    const client = createClient();
    if (!client) return;
    setBusy(true);
