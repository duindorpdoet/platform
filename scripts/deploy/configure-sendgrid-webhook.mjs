...
for (const name of ["APP_ENVIRONMENT", "APP_URL", "SENDGRID_API", "GITHUB_ENV"]) {
...
if (!webhook && webhooks.length >= Number(current.max_allowed ?? 1)) {
  throw new Error("No free SendGrid Event Webhook slot is available; existing non-application webhooks were left untouched.");
}
