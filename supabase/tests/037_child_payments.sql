...
select throws_ok($$select api.admin_child_payment_publish('duindorp-halloween-2026',(select selection from child_pay_test),'25000083-0000-0000-0000-000000000001','https://tikkie.me/pay/child-test','Kindselectie gecontroleerd','child-forbidden')$$,'42501','NOT_AUTHORIZED','a parent cannot publish child Tikkies');
select set_config('request.jwt.claims','{"sub":"f0000000-0000-0000-0000-000000000001","role":"authenticated"}',true);
select throws_ok($$select api.admin_child_payment_publish('duindorp-halloween-2026',(select selection from child_pay_test),'25000083-0000-0000-0000-000000000001','https://tikkie.me.attacker.invalid/pay/test','Kindselectie gecontroleerd','child-host')$$,'22023','VALIDATION_ERROR','lookalike Tikkie hostname is rejected');
select throws_ok($$select api.admin_child_payment_publish('duindorp-halloween-2026',(select jsonb_build_array(selection->0,selection->0) from child_pay_test),'25000083-0000-0000-0000-000000000001','https://tikkie.me/pay/child-test','Kindselectie gecontroleerd','child-duplicate')$$,'22023','VALIDATION_ERROR','duplicate child cannot inflate amount');
select throws_ok($$select api.admin_child_payment_publish('duindorp-halloween-2026',(select selection from child_pay_test),'25000083-0000-0000-0000-000000000003','https://tikkie.me/pay/child-test','Kindselectie gecontroleerd','child-anchor')$$,'22023','INVALID_ANCHOR_CHILD','anchor must belong to selected children');
select throws_ok($$select api.admin_child_payment_publish('duindorp-halloween-2026',(select jsonb_set(selection,'{1,version}','999') from child_pay_test),'25000083-0000-0000-0000-000000000001','https://tikkie.me/pay/child-test','Kindselectie gecontroleerd','child-stale')$$,'40001','STALE_VERSION','one stale child rolls back the complete selection');
reset role;
...
select is((select count(*)::integer from app_private.email_outbox where payload->>'externalUrl'='https://tikkie.me/pay/child-test'),0,'failed publish creates no e-mail');

set local role authenticated;
select set_config('request.jwt.claims','{"sub":"f0000000-0000-0000-0000-000000000001","role":"authenticated"}',true);
select lives_ok($$update child_pay_test set joint=api.admin_child_payment_publish('duindorp-halloween-2026',selection,'25000083-0000-0000-0000-000000000001','https://tikkie.me/pay/child-test','Kindselectie gecontroleerd','child-publish')$$,'two siblings can share one Tikkie');
...
select is(api.admin_child_payment_publish('duindorp-halloween-2026',(select selection from child_pay_test),'25000083-0000-0000-0000-000000000001','https://tikkie.me/pay/child-test','Kindselectie gecontroleerd','child-publish'),(select joint from child_pay_test),'publication replay returns the original batch');
reset role;
...
select lives_ok($$update child_pay_test set single=api.admin_child_payment_publish('duindorp-halloween-2026',third_selection,'25000083-0000-0000-0000-000000000003','https://tikkie.me/pay/third-child','Derde kind apart betalen','third-publish')$$,'third sibling gets a separate Tikkie');
...
select ok(api.registration_snapshot('duindorp-halloween-2026')::text not like '%https://tikkie.me/pay/child-test%','another household snapshot contains no sibling Tikkie URL');
...
select is(app_private.child_payment_batch_payload((select (other_batch->>'id')::uuid from child_pay_test))->>'status','needs_review','child cancellation marks the existing Tikkie unsafe');
select is(app_private.child_payment_batch_payload((select (other_batch->>'id')::uuid from child_pay_test))->>'externalUrl',null::text,'invalidated Tikkie URL is suppressed');
set local role authenticated;
select set_config('request.jwt.claims','{"sub":"a0000000-0000-0000-0000-000000000084","role":"authenticated"}',true);
...
reset role;
select is((select count(*)::integer from app_private.email_outbox where message_type='payment_link_ready' and payload->>'externalUrl' in('https://tikkie.me/pay/child-test','https://tikkie.me/pay/third-child','https://tikkie.me/pay/cancelled-child') and status='suppressed'),3,'settled and cancelled Tikkie mails are suppressed before delivery');
