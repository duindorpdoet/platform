...
select is((select payload->>'registrationReference' from app_private.email_outbox where message_type='payment_link_ready' and payload->>'childBatchId'=(select joint->>'id' from child_pay_test)),(select reference from app_private.registrations where id='22000000-0000-0000-0000-000000000083'),'payment mail includes the registration code');
select is((select payload->'paymentChildDetails' from app_private.email_outbox where message_type='payment_link_ready' and payload->>'childBatchId'=(select joint->>'id' from child_pay_test)),'[{"name":"Betaalkind Alfa 1","amountCents":250},{"name":"Betaalkind Alfa 2","amountCents":250}]'::jsonb,'payment mail includes the exact amount for every selected child');
